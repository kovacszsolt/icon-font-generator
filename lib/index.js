...
function getGeneratorConfig(options) {
    ...
    const config = {
        ...
        verical: options.verical,
        templateOptions: {
            baseTag: options.baseTag.split(',').map((baseTagItem, baseTagIndex) =>
                baseTagItem + '[class^="' + (options.classPrefix || 'icon') + '-' + '"]:before, ' +
                baseTagItem + '[class*="' + (options.classPrefix || 'icon') + '-' + '"]:before' + (baseTagIndex === options.baseTag.split(',').length - 1 ? '' : ',')
            ),
            baseSelector: options.baseSelector || null,
            baseClassNames: classNames.join(' '),
            classPrefix: (options.classPrefix || 'icon') + '-',
            centerVertical: options.centerVertical || false,
